	...
	return {
		...
		getBmpToken: function () {
			return new Promise(function (resolve, reject) {
				$.ajax({
					type: "GET",
					url: "/mifrmdconfiguracion/bpmworkflowruntime/v1/xsrf-token",
					headers: {
						"X-CSRF-Token": "Fetch"
					},
		...
		completeTask: function (taskId, oContext) {
			this.getBmpToken().then(function (oToken) {
				$.ajax({
					url: "/mifrmdconfiguracion/bpmworkflowruntime/v1/task-instances/" + taskId,
					method: "PATCH",
					contentType: "application/json",
					async: false,
					data: JSON.stringify({
		...
		getTaskContextByActivityId: function (instanceID, activityId) {
			return new Promise(function (resolve, reject) {
				this.getTaskByActivityId(instanceID, activityId).then(function (activityContext) {
					...
					$.ajax({
						type: "GET",
						url: "/mifrmdconfiguracion/bpmworkflowruntime/v1/task-instances/" + taskInstanceId + "/context",
						contentType: "application/json",
						dataType: "json",
		...
		getTaskByActivityId: function (instanceID, activityId) {
			return new Promise(function (resolve, reject) {
				$.ajax({
					type: "GET",
					url: "/mifrmdconfiguracion/bpmworkflowruntime/v1/task-instances?workflowInstanceId=" + instanceID,
					contentType: "application/json",
					dataType: "json",
					success: function (result) {
						var activityContext = {};
