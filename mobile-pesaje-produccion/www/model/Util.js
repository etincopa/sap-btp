sap.ui.define(
  ...
  function (MessageBox, Models, Filter, DateFormat, MessageToast) {
    ...
    return {
      ...
      /**
       * Session Storage: se utiliza para almacenamiento temporal y se borra tan pronto como se cierra la aplicación. Utiliza un par clave-valor.
       * Local Storage: se utiliza para almacenamiento persistente y utiliza un par clave-valor. Puede almacenar hasta 5 MB de datos.
       * Cookies: las cookies se utilizan a menudo como tipo de almacenamiento en la web. También utiliza un par clave-valor.
       * Web SQL: Web SQL es Web SQLite. Permite hasta 5 MB de almacenamiento de datos como máximo. Solo Android e iOS tienen WebSQL, mientras que Windows tiene Index DB.
       * Native SQLite Storage: Es un complemento que apuntará al SQLite nativo de la plataforma respectiva. Es fácil de mantener, liviano y flexible. Puede almacenar datos ilimitados.
       * Index DB: Es una base de datos secuencial de la plataforma Windows Phone. No es fácil de mantener como SQLite. También utiliza el almacenamiento de pares clave-valor.
       */
      ...
      saveEntity: function (sId, sObject, bUpd) {
        //Util.saveEntity("Matricula", JSON.stringify(this.getData()), "X");
        /*
        Util.saveEntity(
            "Matricula",
            JSON.stringify({
      ...
      readEntity2: function (sId, bOneWay, that, sEntity) {
        var that = this;
            ...
            open.onsuccess = function () {
              ...
              getEntidad.onsuccess = function () {
                if (getEntidad.result) {
                  resolve(getEntidad.result.content);
                } else {
                  reject(false);
                }
              };
        ...
        if (indexedDB) {
          ...
          open.onsuccess = function () {
            ...
            getEntidad.onsuccess = function () {
              if (getEntidad.result) {
                that.setJsonModel(
                  JSON.parse(getEntidad.result.content),
                  sId + "",
                  bOneWay
                );
                ...
                if (!sFilter.length) {
                  that[sId + "Model"].loadData(sService + sEntity + "");
                } else {
                  that[sId + "Model"].loadData(
                    sService + sEntity + "" + sFilter
                  );
                }
              }
            };
        ...
        if (indexedDB) {
          ...
          open.onsuccess = function () {
            ...
            delOper.onsuccess = function () {
              that[sName + "Model"].loadData(
                sService + sName + "?$filter=Start eq 1"
              );
            };
      ...
      onSyncEntity: function (oEvent) {
        ...
        var oEntity = oEvent
          .getSource()
          .getParent()
          .getBindingContext("StorageSet")
          .getObject();
        var sService = Models.getService();
        that.btnSync = oEvent.getSource();

        sap.m.MessageBox.confirm(
          "¿Desea actualizar los datos de: " + oEntity.Entidad + "?",
          {
            ...
            onClose: function (oAction) {
              if (oAction === sap.m.MessageBox.Action.OK) {
                ...
                if (
                  ...
                  that[oEntity.Entidad + "Model"].loadData(
                    sService + oEntity.Entidad + ""
                  );
                } else {
                  ...
                  //that[oEntity.Entidad + "Model"].loadData(sService + oEntity.Entidad + "?$filter=Start eq 1");
                }
              }
            },
          }
              ...
              if (getEntidad.result && bInit) {
                ...
                switch (sId) {
                  ...
                  case "Calas":
                    ...
                    let oFooter = oColumn.getFooter();
                    if (oFooter) {
                      oFooter.setText(fSumTotal);
                    }
                    break;
                  case "CompEspecie":
                ...
                that[sId + "Model"].loadData(sService + sId + "" + sFilters);
              }
          ...
          open.onsuccess = function () {
            ...
            getInforme.onsuccess = function () {
              if (getInforme.result) {
                ...
                that
                  .getJsonModel("InformeFlota")
                  .setData(JSON.parse(getInforme.result.content));
                ...
                sap.m.MessageBox.show(
                  "No hay informes guardados en borrador.",
                  {
          ...
          getOperaciones.onsuccess = function () {
            if (getOperaciones.result.length) {
              ...
              for (var i = 0; aOperaciones.length > i; i++) {
                ...
                var nVolumen =
                  (aOperaciones[i].content.length * 2) / 1024 / 1024;
      ...
      sortArrayDate: function (list) {
        ...
        function compare(a, b) {
          ...
          var oDateA = that.getDateObjectValue2(
            aDatesA[0],
            aDatesA[1],
            "/",
            ":"
          ...
          var oDateB = that.getDateObjectValue2(
            aDatesB[0],
            aDatesB[1],
            "/",
            ":"
        ...
        if (util.validateDateTime(oEvent)) {
          ...
          if (oDateDescarga >= oDateDescarga2) {
            ...
            sap.m.MessageBox.error(
              "La fecha de fin de descarga debe ser mayor a la fecha de inicio."
            );
          } else {
      ...
      validateArriboDate: function (oEvent, that) {
        ...
        if (util.validateDateTime(oEvent)) {
          ...
          var oDateSalida = util.getDateObject(
            oDPSalidaZonaPesca,
            oTPSalidaZonaPesca,
            "/",
            ":"
          ...
          if (oDateArribo >= oDateDescarga) {
            ...
            that.byId("inpDescarga").setValueState("Error");
            sMensaje =
              "La fecha de descarga debe ser mayor a la fecha de arribo.";
            bErrorArribo = true;
          } else {
      ...
      validateZonaPesca: function (oEvent, that) {
        ...
        if (util.validateDateTime(oEvent)) {
          ...
          var oDateLlegada = util.getDateObject(
            oDPLlegadaZonaPesca,
            oTPLlegadaZonaPesca,
            "/",
            ":"
      ...
      validateZonaPesca2: function (oEvent, that) {
        ...
        if (util.validateDateTime(oEvent)) {
          ...
          if (oDateLlegada >= oDateSalida) {
            ...
            sMensaje =
              "La fecha de salida a la zona de pesca debe ser mayor a la fecha de llegada.";
          } else {
            oDPLlegadaZonaPesca.setValueState(sNone);
            oTPLlegadaZonaPesca.setValueState(sNone);
            oDPSalidaZonaPesca.setValueState(sNone);
          ...
          if (oDateArribo <= oDateSalida) {
            ...
            sMensaje +=
              "La fecha de arribo es menor que la fecha de salida de zona de pesca.";
          } else {
            oDPArribo.setValueState(sNone);
            oTPArribo.setValueState(sNone);
            that.byId("inpArribo").setValueState("None");
      ...
      handleStorageSelect: function (oEvent) {
        ...
        if (sKey === "inf") {
          oBinding.filters = new sap.ui.model.Filter({
            filters: [
              new Filter("Updatable", sap.ui.model.FilterOperator.EQ, ""),
            ],
            and: true,
          });
          ...
          oBinding.filters = new sap.ui.model.Filter({
            filters: [
              new Filter("Updatable", sap.ui.model.FilterOperator.EQ, "X"),
            ],
            and: true,
          });
      ...
      collectArray: function (list, identifier, propToSum) {
        ...
        $.each(list, function (index, element) {
          if (newArr[element[identifier]] == undefined)
            newArr[element[identifier]] = 0;
      ...
      validateNumericDecimal: function (
        oEvent,
        regex,
        rgxReplace,
        bFloat,
        ...
        if (
          iInputValue.indexOf(".") + 1 < iInputValue.length &&
          iInputValue.indexOf(".") !== -1
        ) {
      ...
      handleLiveChange: function (oEvent) {
        ...
        if (text.length === 15 || text.length === 13) {
          aText = text.split(" ");
          sFecha =
            aText[0].substring(0, 4) + aText[0].substring(aText[0].length - 2);
      ...
      esconderColumna: function (sModo, that) {
        ...
        for (var i = 0, n = oColumns.length; i < n; i++) {
          ...
          if (oColumns[i].getHeader().getText() == "% Juv Anch" && sModo) {
            ...
            oColumns[i].getHeader().getText() == "% Juv Anch" &&
            !sModo
          ) {
      ...
      obtenerPlantilla: function (sCentro, sEspecie, sTipoRed, sModo, that) {
        ...
        //Columnas
        if (sTipoRed == "03") {
          var oJuvJ12 = $.sap.calPlantilla12.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          var oJuvJ13 = $.sap.calPlantilla13.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          var oJuvJ2 = $.sap.calPlantilla2.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          ...
          switch (oEspecie) {
            case "16000000":
              that
                .getJsonModel("CalasPlantilla2")
                .setData($.sap.calPlantilla12);
              that.calas.bindColumns(
                "CalasPlantilla2>/",
                that.oColsCalaPlantilla2,
                that
              );
              break;
            case "16000002":
          ...
          var oJuvJ14 = $.sap.calPlantilla14.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          var oJuvJ15 = $.sap.calPlantilla15.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          var oJuvJ3 = $.sap.calPlantilla3.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          ...
          switch (oEspecie) {
            case "16000000":
              that
                .getJsonModel("CalasPlantilla3")
                .setData($.sap.calPlantilla14);
              that.calas.bindColumns(
                "CalasPlantilla3>/",
                that.oColsCalaPlantilla3,
                that
              );
              break;
            case "16000002":
          ...
          var oJuvJ9 = $.sap.calPlantilla9.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          var oJuvJ4 = $.sap.calPlantilla4.find(
            (oPosition) => oPosition.propertyName === "Efici"
          );
          ...
          if (oCentro == "H307") {
            switch (oEspecie) {
              case "16000000":
                that
                  .getJsonModel("CalasPlantilla1")
                  .setData($.sap.calPlantilla8);
                that.calas.bindColumns(
                  "CalasPlantilla1>/",
                  that.oColsCalaPlantilla1,
                  that
                );
                break;
              case "16000002":
      ...
      crearPlantillas: function (that) {
        ...
        function createPlantilla4(
          plantilla1,
          oEditCalasPlantilla1,
          oDataAddBod,
          that
        ) {
          ...
          that.calas.buildItems(plantilla1[21], aCellsEdit, that, 1); //18
          oEditCalasPlantilla1.mAggregations.cells.splice(
            21,
            0,
            oEditCalasPlantilla1.mAggregations.cells[24]
          ); //18 - 21
          oEditCalasPlantilla1.removeCell(23); //22

          plantilla1.splice(22, 0, oDataAddBod.Bod5_Extens); //19
          that.calas.buildItems(plantilla1[22], aCellsEdit, that, 1); //19
          oEditCalasPlantilla1.mAggregations.cells.splice(
            22,
            0,
            oEditCalasPlantilla1.mAggregations.cells[25]
          ); //19 - 22
          oEditCalasPlantilla1.removeCell(26); //23

          plantilla1.splice(23, 0, oDataAddBod.Bod6_Extens); //20
          that.calas.buildItems(plantilla1[23], aCellsEdit, that, 1); //20
          oEditCalasPlantilla1.mAggregations.cells.splice(
            23,
            0,
            oEditCalasPlantilla1.mAggregations.cells[26]
          ); //20 - 23
          oEditCalasPlantilla1.removeCell(27); //24

          plantilla1.splice(24, 0, oDataAddBod.Bod7_Extens); //21
          that.calas.buildItems(plantilla1[24], aCellsEdit, that, 1); //21
          oEditCalasPlantilla1.mAggregations.cells.splice(
            24,
            0,
            oEditCalasPlantilla1.mAggregations.cells[27]
          ); //21 - 24
          oEditCalasPlantilla1.removeCell(28); // 25

          plantilla1.splice(25, 0, oDataAddBod.Bod8_Extens); //22
          that.calas.buildItems(plantilla1[25], aCellsEdit, that, 1); //22
          oEditCalasPlantilla1.mAggregations.cells.splice(
            25,
            0,
            oEditCalasPlantilla1.mAggregations.cells[28]
          ); //22 - 25
          oEditCalasPlantilla1.removeCell(29); //26

          $.sap.calPlantilla4 = JSON.parse(
            JSON.stringify(that.getJsonModel("CalasPlantilla1").getData())
          );
        ...
        function createPlantilla5(
          ...
          plantilla1.splice(21, 0, $.sap.calPlantilla4[21]); //18
          cloneEditPlantilla1.addCell(
            oEditCalasPlantilla1.mAggregations.cells[21]
          ); //18
          cloneEditPlantilla1.mAggregations.cells.splice(
            21,
            0,
            cloneEditPlantilla1.mAggregations.cells[24]
          ); //18 - 21
          cloneEditPlantilla1.removeCell(25); //22

          plantilla1.splice(22, 0, $.sap.calPlantilla4[22]); //19
          cloneEditPlantilla1.addCell(
            oEditCalasPlantilla1.mAggregations.cells[22]
          ); //19
          cloneEditPlantilla1.mAggregations.cells.splice(
            22,
            0,
            cloneEditPlantilla1.mAggregations.cells[25]
          ); //19 - 22
          cloneEditPlantilla1.removeCell(26); //23

          $.sap.calPlantilla5 = JSON.parse(
            JSON.stringify(that.getJsonModel("CalasPlantilla1").getData())
          );
          $.sap.editPlantilla5 = $.extend(true, {}, cloneEditPlantilla1);
        ...
        function createPlantillasExtra(oDataAddBod, that) {
          ...
          that.calas.buildItems(plantilla1[27], aCellsEdit, that, 2); //24
          oEditCalasPlantilla1.mAggregations.cells.splice(
            27,
            0,
            oEditCalasPlantilla1.mAggregations.cells[29]
          ); //24 - 26
          ...
          that.calas.buildItems(plantilla1[24], aCellsEdit, that, 3); //21
          oEditCalasPlantilla1.mAggregations.cells.splice(
            24,
            0,
            oEditCalasPlantilla1.mAggregations.cells[26]
          ...
          var plantilla2 = JSON.parse(
            JSON.stringify(that.getJsonModel("CalasPlantilla2").getData())
          );
          ...
          var plantilla3 = JSON.parse(
            JSON.stringify(that.getJsonModel("CalasPlantilla3").getData())
          );
      ...
      setModoCeldas: function (oPlantilla, sModo) {
        ...
        for (var i = 0, n = oCells.length; i < n; i++) {
          if (
            oCells[i].getId().indexOf("txt") == -1 &&
            oCells[i].getId().indexOf("text") == -1
          ) {
