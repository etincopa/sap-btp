  ...
  function (
    ...
    return Controller.extend(
      "com.medifarma.cp.pesajeimpresionbultosaldo.controller.BaseController",
      {
        ...
        /**
         * Convenience method for getting the object DAO.
         * @public
         * @returns {oDataManagerDAO} the object DAO
         * */
        ...
        oDataCreateDinamic: function (oModel, sEntity, oContent) {
          sap.ui.core.BusyIndicator.show(0);
          return new Promise(function (resolve, reject) {
            oDataService
              .oDataCreate(oModel, sEntity, oContent)
              .then((oData) => {
                sap.ui.core.BusyIndicator.hide();
                var oResult = oData ? oData.data : null;
                if (oResult) {
                  resolve(oResult);
                } else {
                  reject(oData);
          ...
          if (aData) {
            ...
            if (check > from && check < to) {
              ...
              if (sModulo == "VIFA") {
                ...
                oAccion = {
                  totalTraslado: aGroupRole.find(
                    (o) => o.accion === "TRASOWNER"
                  ), //CONTROL TOTAL
                  enpView: aGroupRole.find((o) => o.accion === "TRASVIEW"), //VER EL MODULO
                  reAlmcen: aGroupRole.find((o) => o.accion === "TRASRALM"), //RCEPCIÓN ALM
                  enAlmacen: aGroupRole.find((o) => o.accion === "TRASEALM"), //ENVÍO ALM
                  reProduccion: aGroupRole.find((o) => o.accion === "TRASRPRO"), //ENVÍO ALM
                  enProduccion: aGroupRole.find((o) => o.accion === "TRASEPRO"), //ENVÍO ALM
                  enDevProduccion: aGroupRole.find(
                    (o) => o.accion === "TRASDEVPRD"
                  ), //DEVOLUCION PRODUCCION
                  enDevAlmancen: aGroupRole.find(
                    (o) => o.accion === "TRASDEVALM"
                  ), //DEVOLUCION ALMACEN
                };
              }
            } else {
        ...
        _getFormatQrProd: function (sCode) {
          ...
          if (aQrVar.length === 4) {
            //QR Code
            IdBulto = aQrVar[0].trim();
            CodigoInsumo = aQrVar[1].trim();
            Lote = aQrVar[2].trim();
            Etiqueta = aQrVar[3].trim();
          } else if (aQrVar.length === 1) {
            Etiqueta = aQrVar[0].trim();
          } else {
            return false;
          }
        ...
        getUserInfo: async function () {
          ...
          return new Promise(async function (resolve, reject) {
            sap.Settings.getConfigPropertyMap(
              function (oResult) {
                console.log(oResult);
                resolve(oResult);
              },
              async function (oError) {
                console.log(oError);
                var oResp = await that.logout().catch((oError) => {
          ...
          return new Promise(async function (resolve, reject) {
            sap.logon.Core.deleteRegistration(
              async function (oResult) {
                console.log("user registration deleted");
                await that.loadPage().catch((oError) => {
