  ...
  function (
    ...
    var that,
      ...
      CONSTANT = {
        ...
        OPEPE: "PAOPEPE", //PENDIENTE PESAJE
        OPESA: "PAOPESA", //PESANDO EN SALA
    ...
    return BaseController.extend(
      "com.medifarma.cp.manejopedidos.controller.centralPesadas.SeguimientoPickingPedido",
      {
        //Formatter
        formatter: formatter,
        /**-----------------------------------------------*/
        ...
        onInit: async function () {
          ...
          that.oRouter
            .getTarget("TargetSeguimientoPickingPedido")
        ...
        onLiberarInsumo: async function (oEvent) {
          ...
          if (aInsumo && aInsumo.length) {
             ...
             */
            if ([CONSTANT.IATPI].includes(oInsumo.insumoEstado)) {
              ...
              /**
               * Para liberar el insumo se envia todos los registros relacionados al insumo que se pretende liberar:
               * Pedido, Orden, Fraccion, Insumo, Lote del Insumo, Sub-Fraccion
               */
              ...
              if (aResp && aResp.statusCode == 201) {
                ...
                if (aInsumoAtencion.length) {
                   ...
                   * actualizar al estados PICKING todas las ordenes afectadas por la liberacion
                   */
                  var oPedidoFormat =
        ...
        onTararEnteroPress: function (oEvent) {
          let oItem = oEvent
            .getSource()
            .getBindingContext("FraccionDetalleModel");
          ...
          that
            .getModel("FraccionDetalleModel")
            .setProperty("/sPathBultoTara", oItem.getPath());

          var oObject = oItem.getObject();

          /**
           * Verifica si el bulto a tarar tenga la unida de medida KG y G
           */
          if (["KG", "G"].includes(oObject.UnidadM.toUpperCase())) {
            oObject.TaraNew = oObject.Tara;

            that
              .getModel("FraccionDetalleModel")
              .setProperty("/oBultoTara", oObject);
        ...
        onChangeBultoTara: function (oEvent, sAction) {
          var oModel = that.getModel("FraccionDetalleModel"),
            oBulto = oModel.getProperty("/oBultoTara"),
            aBultos = [];

          if (sAction == "SELECCION") {
            var sPath = oModel.getProperty("/sPathBultoTara");
            that.getModel("FraccionDetalleModel").setProperty(sPath, oBulto);
            aBultos = [oBulto];
          } else {
            aBultos = oModel.getProperty("/Bultos");
          }

          if (+oBulto.TaraNew > 0) {
            that._changeTara(aBultos, oBulto);
          } else {
            return MessageToast.show("Ingrese un valor mayor a CERO.");
          }
        },
        ...
        onImprimir: async function (oEvent) {
          ...
          /**
           * Valida que se haya seleccionada una impresora
           */
            ...
            if (oAction === "BULTO") {
              if (oBulto.Tipo != "ENTERO") {
                return MessageBox.show(
                  "El bulto selecionado no es un BULTO ENTERO."
                );
              }
              ...
              /**
               * Obtiene tolos los registros de los bultos de tipo ENTERO
               */
              ...
              if (aBultoEntero.length <= 0) {
                return MessageBox.show("No se encontraron Bultos Enteros.");
              }

              /**
               * Envia los bultos a la cola de impresion
               */
              that._sendPrint({
        ...
        onExportXLSReport: async function (oEvent) {
          ...
          wb.Props = {
            Title: "SEGIMIENTO PICKING PEDIDO",
            Subject: "TITAN",
            Author: "MEDIFARMA",
          ...
          XLSX.writeFile(wb, "SEGIMIENTO PICKING PEDIDO.xlsx");
        },
          ...
          oDataService
            ...
            .then((oResult) => {
              ...
              for (var key0 in aInsumos) {
                ...
                if (!(cantAtendidaEntero >= +oInsumo.sugerido)) {
                  for (var key in aBulto) {
                    var oItem = aBulto[key];
        ...
        _changeTara: function (aBultos, oBulto) {
          var that = this;
          sap.ui.core.BusyIndicator.show(0);
          var aBultoFilter = [];

          /**
           * Construye la estructura de datos de los bultos a enviar para el cambio de valor de la tara
           */
          aBultos.forEach((oItem, i) => {
            if (oItem.Tipo === "ENTERO") {
              var o = Object.assign({}, oItem);
              o.Tara = oBulto.TaraNew;
              o.HoraAte = formatter.formatPTHMS(
                formatter.getMsToHMS(o.HoraAte)
              );
              o.FechaAte =
                formatter.getFormatShortDateYMD(o.FechaAte) +
                "T00:00:00.0000000";
              o.Horac = formatter.formatPTHMS(formatter.getMsToHMS(o.Horac));

              delete o.__metadata;
              delete o.TaraNew;
              delete o.FechaHoraAte;

              delete o.UsuarioVerificadorBulto;
              delete o.FechaVerificacionBulto;
              delete o.EntregaFisFecha;
              delete o.EntregaFisHora;
              delete o.StatusP;
              aBultoFilter.push(o);
            }
          });
          ...
          /**
           * El servicio acSetEtiquetaErp actualiza internamente los registros de las taras
           * haciendo uso del servicio ERP POST  AtendidoSet
           */
          var sDialog = "Tarar";
          oDataService
            .oDataCreate(that.oModel, "/acSetEtiquetaErp", {
              oBody: JSON.stringify(aBultoFilter),
            })
            .then((oResult) => {
              sap.ui.core.BusyIndicator.hide();
              if (oResult.statusCode == "200") {
                var acSetEtiquetaErp = oResult.data.acSetEtiquetaErp;
                if (acSetEtiquetaErp.data && acSetEtiquetaErp.data.error) {
                  var sMsg = acSetEtiquetaErp.data.error.message.value;
                  return MessageBox.error(sMsg);
                } else {
                  MessageBox.information("Tara Actualizada con exito");
                }
              } else {
                MessageBox.warning("Verifique si la tara fue actualiza.");
              }
              that._getInsumoPedidoSelect(that.oPedidoSelect);
              that.onCloseFragment(sDialog);
            })
            .catch((oError) => {
              that.onCloseFragment(sDialog);
              sap.ui.core.BusyIndicator.hide();
        ...
        _sendPrint: function (oData) {
           ...
           * - Envia los registos al servicio de fnSendPrintBulto el cual construira la estructura
           *  de las variables necesaria para la plantilla del bulto y guardara estos registros en la
           *  tabla de cola de impresion
           */
          var oPrint = JSON.parse(window.localStorage.getItem("oImpresora"));
          that
            ._getODataDinamic(
