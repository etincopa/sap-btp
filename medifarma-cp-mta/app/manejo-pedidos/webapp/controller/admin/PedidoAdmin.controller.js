  ...
  function (
    ...
    var that,
      ...
      CONSTANT = {
         ...
         * ESTADOS
         */
        ESTADO_CP_PEDIDO: "ESTADO_CP_PEDIDO",
        ESTADO_CP_ORDEN: "ESTADO_CP_ORDEN",
        ESTADO_CP_INSUMO: "ESTADO_CP_INSUMO",

        ESTADO_AM_PEDIDO: "ESTADO_AM_PEDIDO",
        ESTADO_AM_ORDEN: "ESTADO_AM_ORDEN",
        ESTADO_AM_INSUMO: "ESTADO_AM_INSUMO",
    ...
    return BaseController.extend(
      "com.medifarma.cp.manejopedidos.controller.admin.PedidoAdmin",
      {
        formatter: formatter,
        ...
        onInit: function () {
          ...
          //Lista de ordenes selecionadas (Dialog)
          that.setModel(new JSONModel([]), "TempOrdenListModel");
          ...
          that
            ...
            .then((oResp) => {
              if (oResp) {
                 ...
                 * CPPA: CENTRAL DE PESADAS PEDIDOS DE ATENCION
                 */
                ...
                /**
                 * Obtiene la lista de tipo de pedidos
                 */
                that._getTipoPedido();
              }
              return false;
        ...
        onShowHistory: function (oEvent) {
          ...
          if (iIndex < 0) {
            ...
            that
              ...
              .then((aPedidoHistory) => {
                that._openDialogDinamic("HistorialCp");
        ...
        onRowSelectPedido: function (oEvent) {
          ...
          that.getModel("OrdenSelectedItemsModel").setData([]);
          that.getModel("OrdenSelectedItemsModel").refresh(true);
        ...
        onRowSelectOrden: function (oEvent) {
          var that = this;
          that.oPedidoSelect = null;
          //var oPickingModel = oView.getModel("PickingModel");
          var oTable = this.byId("idOrdenList");
          var iIndex = oTable.getSelectedIndex();
          var sPath;
          ...
          if (iIndex < 0) {
            ...
            try {
              if (oData) {
                that.oPedidoSelect = oData;
                that._getInsumoOrdenSelect(oData);
              }
            } catch (oError) {
              console.log("Error All Init");
              sap.ui.core.BusyIndicator.hide();
        ...
        onCreate: function (oEvent) {
          ...
          this._openDialogDinamic("TipoPedidoCp");
        },

        /**
         * EVENTS: PedidoAtencionCPUsuarioAsignado.fragment
         */
        ...
        onSearchUser: function (oEvent) {
          ...
          if (!oTable) {
            oTable = Fragment.byId(
              "frgPedidoAtencionCPUsuarioAsignado",
        ...
        /**
         * EVENTS: CrearPedidoAtencionCP.fragment
         */
        ...
        onCreatePA: function (oEvent) {
          ...
          if (oParameter.keyCentro && oParameter.keySala) {
            /**
             * Construir estructuras de las entidades para crear los registros del pedido
             */
          ...
          /**
           * Se procede a crear el pedido y se notifica el correcto registro del pedido
           */
        ...
        /**
         * EVENTS: CrearPedidoAtencionIndividual.fragment
         */
        ...
        onSearchMatchCodeOrden: async function (oEvent) {
          sap.ui.core.BusyIndicator.show(0);
          that.setModel(new JSONModel([]), "McOrdenModel");
          var dDate = Fragment.byId("frgMatchCodeOrden", "idDateFechaOrdenMC");
        ...
        onFilterMatchCodeOrden: function (oEvent) {
          ...
          var oTable = that.getView().byId("idTableMatchCodeOrden");
          if (!oTable) {
            var oTable = Fragment.byId(
              "frgMatchCodeOrden",
              "idTableMatchCodeOrden"
            );
          }
        ...
        onFilterMatchCodeInsumo: function (oEvent) {
          ...
          var oTable = that.getView().byId("idTableMatchCodeInsumo");
          if (!oTable) {
            var oTable = Fragment.byId(
              "frgMatchCodeInsumo",
              "idTableMatchCodeInsumo"
            );
          }
          oTable.getBinding().filter(aFilters);
        },
        ...
        onFilterMatchCodeLote: function (oEvent) {
          ...
          var oTable = that.getView().byId("idTableMatchCodeLote");
          if (!oTable) {
            var oTable = Fragment.byId(
              "frgMatchCodeLote",
              "idTableMatchCodeLote"
            );
          }
          oTable.getBinding().filter(aFilters);
        },
        ...
        onAddItemsIndivToList: async function (oEvent) {
          ...
          if (oInsumo) {
            ...
            if (aPedido.length && aPedido.length > 0) {
              ...
              that._openDialogDinamic("PedidoAtencionCPView");
            ...
            try {
              ...
              if (aOrdenBuild.length) {
                ...
                if (aReservaBuild && aReservaBuild.length > 0) {
                  that._openDialogDinamic("MatchCodeInsumo");
                } else {
                  MessageBox.error(
        ...
        onCreatePAI: function (oEvent) {
          ...
          if (
            ...
            /**
             * Construir estructuras de las entidades para crear los registros del nuevo pedido
             */
          ...
          /**
           * Se procede a crear el pedido y se notifica el correcto registro del nuevo pedido
           */
          var aPromise = [];
          for (var oEntity of aEntityData) {
            var sEntity = oEntity.sEntity;
          ...
          Promise.all(aPromise)
            ...
            .finally((oFinal) => {
              ...
              sap.ui.core.BusyIndicator.hide();
              Fragment.byId(
                "frgCrearPedidoAtencionIndividual",
        ...
        /**
         * EVENTS: CrearPedidoAtencionReservaIDE.fragment
         */
        ...
        onSelectReservaDisplayInsumo: function (oEvent) {
          if (
            ...
            var oFragment = Fragment.byId(
              "frgCrearPedidoAtencionReservaIDE",
        ...
        onDeleteItemsReservas: function (oEvent) {
          var oBindingContext = oEvent
            .getSource()
            .getBindingContext("ReservaListModel"); //.getObject();
        ...
        onAddReservaToList: function (oEvent) {
          ...
          /**
           * Evalua si la Orden e insumo aun no haya sido agregada anteriormente
           */
          var bExist = false;
          if (aReserva) {
          ...
          if (!bExist) {
            ...
            /**
             * Se valida si la orden e insumo seleccionada existe ya en otro pedido creada anteriormente,
             * para obtener los pedidos anteriores que tiene asignado la orden e insumo
             * se hace uso de la vista VIEW_SEGUIMIENTO_DETALLE,
             * vista que devuelve todo el detalle en genera del pedido la cual esta conformada por
             * PEDIDO -> FRACCION -> INSUMOS (MATERIALES)
             */
            that
        ...
        onAddOrdenToList: function (oEvent) {
          ...
          if (oRegistro) {
            ...
            /**
             * Evalua si la Orden aun no haya sido agregada anteriormente.
             */
            var bExist = false;
            if (aTempOrdenList) {
            ...
            if (!bExist) {
              ...
              /**
               * Se valida si la orden seleccionada existe ya en otro pedido ya creada anteriormente.
               * Si la orden ya existe en otra orden mostrar un mensaje no restrictivo indicando los
               * pedidos que tienen asociados la orden.
               */
              that
                ._getODataDinamicSetModel(
                  null,
          ...
          if (aTempOrdenItemsList.length) {
            try {
              ...
              try {
                ...
                /**
                 * Obtener todas las ordenes seleccionadas para su evaluacion
                 */
                var aOrden = await that._getODataDinamic(
                  that.oModelErp,
                  "OrdenSet",
                ...
                if (aOrden && aOrden.length) {
                  ...
                  /**
                   * Obtener todas las fracciones de las Ordenes eleccionadas e Insumos.
                   */
                  var aFraccion = await that._getODataDinamic(
                    that.oModelErp,
                    "FraccionamientoSet",
                  ...
                  /**
                   * Obtener todos los Insumos de las ordenes seleccionadas.
                   */
                  var aReserva = await that._getODataDinamic(
                    that.oModelErp,
                    "ReservaSet",
                  ...
                  if (
                    ...
                    /**
                     * Se verifica y se obtiene las fracciones de los insumos:
                     * - Verificar que todas las ordenes tengan Lote (Charg2)
                     * - Se realiza el cruce de informacion entre los servicios ReservaSet y FraccionamientoSet
                     * para determinar las sub-fracciones de los insumos y  fracciones de las ordenes
                     */
                    for (var oItem of aOrden) {
                      var aFraccionFilter = aFraccion.filter((o) => {
                        return (
                ...
                if (aOrdenBuild.length) {
                  ...
                  Fragment.byId("frgCrearPedidoAtencionCP", "idTableInsumosCP")
                    .getBinding()
                    .filter([new Filter("NumReserva", EQ, "")]);
                  sap.ui.core.BusyIndicator.hide();
                  Fragment.byId("frg" + "OrdenCp", "dialogOrden").close();
        ...
        onSearchAgregarOrden: function (oEvent) {
          var oTableAgregarOrdenes = Fragment.byId(
              "frg" + "OrdenCp",
              "idOrdenList"
            ),
        ...
        onRestoreFiltersAgregarOrden: function () {
          ...
          Fragment.byId("frg" + "OrdenCp", "idOrdenList")
            .getBinding()
            .filter([]);
          Fragment.byId("frg" + "OrdenCp", "idOrdenList")
            .getBinding()
        ...
        onSearchFilter: async function (oEvent) {
          ...
          var oFilter = "",
            ...
            aFiltersOrdenes = [],
            oFinalFilter;
          oEvent.getParameter("selectionSet").forEach((oElementoFiltro) => {
            oFilter = "";
            if (
            ...
            if (
              ...
              var aUsuarioPedido = this.getView()
          ...
          if (aFiltersPedidos.length !== 0) {
            ...
            if (aPedidoFilter && aPedidoFilter.length) {
              ...
              aFiltersPedidos.push(new Filter("pedidoId", EQ, "NONE"));
            }
        ...
        onPressAceptar: function (oEvent) {
          ...
          if (
            ...
            sDialog = "CrearPedidoAtencionIndividual";
            that.setModel(new JSONModel([]), "McInsumoModel");
            that.setModel(new JSONModel([]), "McLoteModel");
          } else if (
            [
            ...
            sDialog = "CrearPedidoAtencionReservaIDE";
          } else {
            return;
          }
        ...
        onChangeStatusGeneral: function (oEvent, sNivel) {
          var that = this;
          ...
          var infoEstado = "";
          var infoObjeto = "";
          if (!this.ChangeStatusGeneralDialog) {
            if (sNivel == "PEDIDO") {
              var oTable = this.byId("idPedidoList");
              var iIndex = oTable.getSelectedIndex();
              var sPath;
              ...
              if (iIndex < 0) {
                ...
                sPath = oTable.getContextByIndex(iIndex).getPath();
                oData = oTable.getModel().getContext(sPath).getObject();

                infoPedido = oData.numPedido;
                infoObjeto = infoPedido;
                infoEstado = oData.Estado_contenido;
              }
              if (oData.Tipo_codigo == "PCP") {
                aEstadoLevel = aMaestraList[CONSTANT.ESTADO_CP_PEDIDO];
              } else {
                aEstadoLevel = aMaestraList[CONSTANT.ESTADO_AM_PEDIDO];
              }
            } else if (sNivel == "FRACCION") {
              var oTable = this.byId("idOrdenList");
              var iIndex = oTable.getSelectedIndex();
              var sPath;
              ...
              if (iIndex < 0) {
                ...
                oData = oTable
                  ...
                  .getObject();
                infoPedido = oData.numPedido;
                infoObjeto = oData.numOrden + " ( " + oData.numFraccion + " )";
                infoEstado = oData.estadoOrden;
              }

              if (oData.tipoCodigo == "PCP") {
                aEstadoLevel = aMaestraList[CONSTANT.ESTADO_CP_ORDEN];
              } else {
                aEstadoLevel = aMaestraList[CONSTANT.ESTADO_AM_ORDEN];
              }
            } else if (sNivel == "INSUMO") {
              var oTable = this.byId("idTableInsumos");
              var iIndex = oTable.getSelectedIndex();
              var sPath;
              ...
              if (iIndex < 0) {
                ...
                oData = oTable
                  ...
                  .getContext(sPath)
                  .getObject();
                infoPedido = oData.pedidoNumero;
                infoObjeto =
                  oData.codigoInsumo +
                  " - " +
                  oData.lote +
                  " ( " +
                  oData.numSubFraccion +
                  " )";
                infoEstado = oData.insumoEstadoDes;
              }

              if (oData.pedidoTipo == "PCP") {
                aEstadoLevel = aMaestraList[CONSTANT.ESTADO_CP_INSUMO];
              } else {
                aEstadoLevel = aMaestraList[CONSTANT.ESTADO_AM_INSUMO];
              }
            }

            that.setModel(new JSONModel(aEstadoLevel), "EstadoLevelModel");
            this.ChangeStatusGeneralDialog = new Dialog({
                ...
                new HorizontalLayout({
                  content: [
                    new VerticalLayout({
                      width: "120px",
                      content: [
                        new Text({ text: "Pedido: " }),
                        new Text({ text: "Objeto: " }),
                        new Text({ text: "Estado: " }),
                      ],
                    }),
                    new VerticalLayout({
                      content: [
                        new Text({ text: infoPedido }),
                        new Text({ text: sNivel + ": " + infoObjeto }),
                        new Text({ text: infoEstado }),
                      ],
                    }),
                  ],
                }),
                new Select("StatusLevelId", {
                  width: "100%",
                  items: {
                    path: "EstadoLevelModel>/",
                    template: new Item({
                      key: "{EstadoLevelModel>iMaestraId}",
                      text: "{EstadoLevelModel>contenido}",
                    }),
                  },
                }),
              ],
              beginButton: new Button({
                ...
                press: async function () {
                  //console.log(sNivel);
                  //console.log(oData);
                  var iMaestraId = parseInt(
                    Core.byId("StatusLevelId").getSelectedItem().getKey()
                  );
                  ...
                  var oKey = {};
                  if (sNivel == "PEDIDO") {
                    oKey = that.oModel.createKey("/PEDIDO", {
                      pedidoId: oData.pedidoId,
                    });
                  } else if (sNivel == "FRACCION") {
                    oKey = that.oModel.createKey("/ORDEN_FRACCION", {
                      ordenFraccionId: oData.ordenFraccionId,
                    });
                  } else if (sNivel == "INSUMO") {
                    oKey = that.oModel.createKey("/ORDEN_DETALLE", {
                      ordenDetalleId: oData.ordenDetalleId,
                    });
                  }

                  var oEntity = {
                    oEstado_iMaestraId: iMaestraId, // Nuevo estado
                  };
                  ...
                  Promise.all(aPromise)
                    .then((oResp) => {
                      ...
                      //that.byId("idOrdenList").getBinding().refresh(true);
                      //that.byId("idTableInsumos").getBinding().refresh(true);

                      that.sendPedidoToErp(that, oData.pedidoId);
                      sap.ui.core.BusyIndicator.hide();

                      this.ChangeStatusGeneralDialog.destroy();
                      this.ChangeStatusGeneralDialog = null;
                    })
                    .catch((oError) => {
                      sap.ui.core.BusyIndicator.hide();
                      MessageBox.error(that._getI18nText("msgActualizaErr"));
            ...
            this.getView().addDependent(this.ChangeStatusGeneralDialog);
          }

          this.ChangeStatusGeneralDialog.open();
        },

        /**
         * EVENT: PedidoAtencionCPView.fragment
         */
        ...
        onExportXLSReport: async function (oEvent) {
          ...
          XLSX.writeFile(wb, "SEGIMIENTO PEDIDO.xlsx");
        },
        ...
        _getInsumoOrdenSelect: function (oData) {
          var oView = that.getView();
                ...
                if (oInsumo.pedidoTipo == "PCP") {
                  for (var key in aBulto) {
                    ...
                    if (
                      ...
                      if (["ENTERO"].includes(oItem.Tipo)) {
                        ...
                        cantAtendidaFracc =
                          cantAtendidaFracc + +oItem.CantidadA;
                      }
                  ...
                  for (var key in aBulto) {
                    ...
                    if (
                      ...
                      oItem.CodigoInsumo == oInsumo.codigoInsumo &&
                      oItem.Lote == oInsumo.lote &&
                      ["ENTERO_ALM", "SALDO_FRAC_ALM", "SALDO_ALM"].includes(
                        oItem.Tipo
                      )
                    ) {
                      /**
        ...
        _pedidoShowUserAssignments: function (oEvent, sKeyPedido) {
          ...
          that
            ...
            .then((oDataEntity) => {
              ...
              that._openDialogDinamic("PedidoAtencionCPUsuarioAsignado");

              /**
               * Obtener listado de usuario cuyo rol es:
               * Auxiliar de Almacén de Insumos
               */
              var aFilters = [];
              ...
              /**
               * Obtener la lista de usuarios haciendo uso del servicio odata hana: UsuarioRol
               */
        ...
        _pedidoChangeStatus: async function (oEvent, sKeyPedido, sAction) {
          ...
          var aEstadoPedido = aMaestra[CONSTANT.ESTADO_CP_PEDIDO];
          ...
          for (var key in aOrdenT) {
            ...
            for (var oOrden of aOrdenT[key]) {
              ...
              for (var oFraccion of aFraccionT) {
                ...
                oNewFraccion.oSala_salaPesajeId =
                  oParam.tipoPedidoParams.keySala;
        ...
        onOpenDialogOR: async function (oEvent, sTipo) {
          ...
          if (aComprometida && aComprometida.length) {
            ...
            this._openDialogDinamic("OrdenReservadaCp");
          }
        },
        ...
        _reservaFilter: function (aReserva) {
          if (!(aReserva && aReserva.length)) return aReserva;
        ...
        _getConstant: function () {
          ...
          var oMaestraList = that.getModel("MaestraModel").getData();
          var oConstant = {
            oEstadoPedido: oMaestraList[CONSTANT.ESTADO_CP_PEDIDO].find(
              //ESTADO CP PEDIDO
              (o) => o.codigo === CONSTANT.PPEND
            ),
            oEstadoOrden: oMaestraList[CONSTANT.ESTADO_CP_ORDEN].find(
              //ESTADO CP PEDIDO DETALLE (ORDEN)
              (o) => o.codigo === CONSTANT.OPEND
            ),
            oEstadoOrdenDet: oMaestraList[CONSTANT.ESTADO_CP_INSUMO].find(
        ...
        _getEstadoPedido: function () {
          ...
          var aEstadoPedido = aMaestra[CONSTANT.ESTADO_CP_PEDIDO];
          var aEstadoOrden = aMaestra[CONSTANT.ESTADO_CP_ORDEN];
          var aEstadoInsumo = aMaestra[CONSTANT.ESTADO_CP_INSUMO];
