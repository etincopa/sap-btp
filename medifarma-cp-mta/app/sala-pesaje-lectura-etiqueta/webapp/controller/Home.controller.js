sap.ui.define(
  [
    ...
    "com/medifarma/cp/salapesajelecturaetiqueta/services/Maestro",
    "com/medifarma/cp/salapesajelecturaetiqueta/services/SalaPesaje",
    "com/medifarma/cp/salapesajelecturaetiqueta/services/Log",
    "sap/ui/core/BusyIndicator",
    "sap/m/MessageBox",
    "sap/m/MessageToast",
    "com/medifarma/cp/salapesajelecturaetiqueta/util/http",
  ...
  function (
    ...
    "use strict";
    const rootPath = "com.medifarma.cp.salapesajelecturaetiqueta";
    const EdmType = exportLibrary.EdmType;

    return BaseController.extend(
      "com.medifarma.cp.salapesajelecturaetiqueta.controller.Home",
      {
        onInit: function () {
          this.init();
        ...
        onExportar: function (oEvent) {
          ...
          _aColumns.forEach(function (item) {
            if (item.getSortProperty() != "") {
              ...
              if (
                oColumn.property == "inicioLectEtiqueta" ||
                oColumn.property == "finLectEtiqueta"
              ) {
          ...
          oSettings = {
            workbook: {
              ...
              context: {
                sheetName: "Lectura Etiqueta",
              },
            },
            dataSource: aSala,
            fileName: "Lectura_Etiqueta.xlsx",
        ...
        onPressSave: async function (oEvent) {
          ...
          try {
            const oSala = this.oLocalModel.getProperty("/Sala");
            const aRequeridos = ["oEstadoLecturaEtiqueta_iMaestraId"];
            ...
            const oEstado = aEstados.find(
              (e) =>
                e.iMaestraId ===
                parseInt(oSala.oEstadoLecturaEtiqueta_iMaestraId)
            );

            if (oEstado.codigo == "HABIL") {
              oSala.inicioLectEtiqueta = new Date();
            } else {
              oSala.inicioLectEtiqueta = null;
              oSala.finLectEtiqueta = null;
            }
            ...
            if (oSala.inicioLectEtiqueta != null) {
              const dFechaInicio = new Date(oSala.inicioLectEtiqueta);
              const dNuevaFecha = new Date(
                dFechaInicio.valueOf() + oSala.horaLectEtiqueta * 60 * 60 * 1000
              );
              oSala.finLectEtiqueta = dNuevaFecha;
            }
