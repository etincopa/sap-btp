sap.ui.define(["sap/ui/model/json/JSONModel"], function (JSONModel) {
  ...
  return {
            ...
            if (oData.results) {
              ...
              if (oData.results.length === 1000) {
                while (aListPasosLength === 1000) {
                  let aListPasoSkipData = await oThat.oDataReadSkip(
                    oModel,
                    sEntity,
                    skip,
                    oUrlParameters,
                  ...
                  resultFinal.results = resultFinal.results.concat(
                    aListPasoSkipData.results
                  );
                  skip = skip + 1000;
                }
              }
    ...
    getDataAjax: function (sFilter) {
      return new Promise(function (resolve, reject) {
        var uri =
          "/saperp/sap/opu/odata/sap/Z_PP_PRODUCCION_SRV/ProduccionSet?$filter=Werks eq '" +
          sFilter +
          "'";
        $.ajax({
          type: "GET",
