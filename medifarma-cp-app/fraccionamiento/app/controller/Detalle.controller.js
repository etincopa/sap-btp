    ...
    return Controller.extend("mif.cp.fraccionamiento.controller.Detalle", {
      ...
      cargarDatos: async function (grupoOrdenConsolidadoId) {
        ...
        try {
          ...
          const oEstadoPendiente = aEstadosOrden.find(
            (e) => e.codigo == "PAIPEPE"
          );
          const oEstadoPesando = aEstadosOrden.find(
            (e) => e.codigo == "PAIPESA"
          );
          var iEstadoConsolidadoId = this.oLocalModel.getProperty(
            "/EstadoConsolidadoId"
          );
          ...
          aEstadosOrden = aEstadosOrden.filter(
            (d) => oEstadoPesando.iMaestraId != d.iMaestraId
          );
      ...
      _cargarFraccionamientos: async function (grupoOrdenConsolidadoId) {
        ...
        var iConsolidadoSalaId = this.oLocalModel.getProperty(
          "/Consolidado_salaId"
        );
          ...
          if (oItemSelect) {
            ...
            if (oAtendido) {
              ...
              if (oResp) {
                ...
                await this._cargarFraccionamientos(
                  oConsolidado.grupoOrdenConsolidadoId
                );
      ...
      onEliminarPress: async function () {
        ...
        if (this.oFraccionamiento !== null) {
          oDetalle_Fraccionamientos = oDetalle_Fraccionamientos.filter(
            (d) => d.ordenNumero == this.oFraccionamiento.ordenNumero
          );

          if (
            oDetalle_Fraccionamientos.length !=
            this.oFraccionamiento.numSubFraccion
          ) {
            BusyIndicator.hide();
            MessageBox.msgError(
                ...
                if (sAction == "SI") {
                  if (that.oFraccionamiento.numSubFraccion) {
                    const iSplicePos = oDetalle_Fraccionamientos.findIndex(
                      (e) =>
                        e.numSubFraccion == that.oFraccionamiento.numSubFraccion
                    );
                    ...
                    await that._cargarFraccionamientos(
                      that.grupoOrdenConsolidadoId
                    );
                  } else {
                    that.oFraccionamiento.activo = false;
                    BusyIndicator.show(0);
          ...
          if (oRowSelect) {
            ...
            if (aReserva && !aReserva.error) {
              oReserva = aReserva.find(
                (d) =>
                  d.Matnr == oRowSelect.codigoInsumo &&
                  d.Charg == oRowSelect.loteInsumo
              );
            }
            ...
            this.oLocalModel.setProperty(
              "/Detalle_Eliminar",
              navigator.onLine && this.oFraccionamiento.FracDuplicado == "X"
            );
      ...
      onRegresarPress: function () {
        ...
        this.oLocalModel.setProperty(
          "localModel>/Consolidado_estado",
          iEstadoConsolidadoId
        );
      ...
      onAfterRendering: async function(){
        try {
          BusyIndicator.show(0);
          console.log("ENTRA AFTER RENDERING");
          var maestra = this.oLocalModel.getProperty("/Maestra");
        if(!maestra &&  this.grupoOrdenConsolidadoId){
          console.log("DATOS NO CARGADOS");
          await this.cargarDatos(this.grupoOrdenConsolidadoId);
          var tituloPagina = this.oResourceBundle.getText("atencionOrden");
          this.oLocalModel.setProperty("/tituloPagina", tituloPagina);
        }
          BusyIndicator.hide();
        } catch (error) {
          console.log("ERROR AFTER RENDERING: ", error);
          BusyIndicator.hide();
        }
      },
    });
