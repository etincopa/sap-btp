sap.ui.define(["sap/ui/model/json/JSONModel", "sap/ui/Device"], function (JSONModel, Device) {
    ...
    return {
        ...
        getSunatStatus: function (oThat, oData, iRetry = 0) {
            try {
                let that = this;
                    ...
                    that.consultSunat(oThat, "/sunatApiResponse", oData)
                        ...
                        .catch((err) => {
                            ...
                            if (iRetry === that.iMaxRetryForDocValidation) {
                                ...
                                return that.getSunatStatus(oThat, oData, iRetry).then((iResult) => {
                                    resolve(iResult);
                                }).catch((err) => {
                                    reject(err);
                                });
        ...
        consultSunat: function (oThat, sUrl, aData) {
            try {
                sUrl = oThat.getOwnerComponent().getManifestObject().resolveUri("odataSunat/api/v1/suppliers" + sUrl);
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        type: "POST",
                        crossDomain: true,
