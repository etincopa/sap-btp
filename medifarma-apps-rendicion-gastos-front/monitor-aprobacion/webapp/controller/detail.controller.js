sap.ui.define([
	...
	"use strict";
	$.nombreDocumento = "";
	$.nombreDocumentoConsignacion = "Solicitud de Traspaso";
	$.nombreDocumentoConsumo = "Reserva";
	return baseController.extend("com.everis.monitorDocumentos.controller.detail", {
		inputId: "",
		sPathTbl: "",
		sDeleteIndex: "",
		oRegistroModel: "",
		handleRouteMatched: function (oEvent) {
			var sTexto = oEvent.getParameters().data.idDocumento,
				model = new JSONModel();
			var that = this;
			var temp = sTexto.split("-");
			
			if(temp[1] === "CONSUMO"){
					$.nombreDocumento = $.nombreDocumentoConsumo;
			}else{
					$.nombreDocumento = $.nombreDocumentoConsignacion;
			}
			var idReserva = temp[0];
		...
		getReservaModel: function (idReserva) {
			...
			var query = "/zreservaSet(Rsnum='" + idReserva + "')?$expand=ToReservaPosciones";
			var queryTable = "/zreservaSet('" + idReserva + "')/ToReservaPosciones";
			this.getOwnerComponent().getModel("oBackendDataModel").read(query, {
				beforeSend: function () {
				...
				success: function (res) {
					...
					if (res.Sobkz === "") {
						...
						res.nombreDocumento = "Reserva N°";
					} else {
						res.isConsignacion = true;
						res.nombreDocumento = "Solicitud de Traspaso N°";
					}
					if (res.Kostl !== "") {
						res.Rstyp = "K";
				...
				error: function (err) {
					...
					if (err.responseText !== undefined) {
						...
						if (e.error.code === "/IWBEP/CM_MGW_RT/020") {
							msj = $.nombreDocumento+" N°" + idReserva + " ha sido eliminada de SAP.";
			...
			this.getOwnerComponent().getModel("oBackendDataModel").read(queryTable, {
				beforeSend: function () {
					that.getView().setBusyIndicatorDelay(0);
					that.getView().setBusy(true);
				},
