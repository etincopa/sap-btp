sap.ui.define([
	...
	return Controller.extend("com.everis.monitorDocumentos.controller.crearGastoFondoFijo", {
		proceso: "",
		regla: "",
		Nivel: "1",
					...
					oThes.getOwnerComponent().getModel("oDataModelFondoFijo").read("/IasSet('" + oUserIas.name + "')", {
						...
						success: function (result) {
							if (result.Bukrs !== null && result.Bukrs !== "" && result.Bukrs !== undefined) {
								...
								oThes.getView().byId("idFechaFactura").setMaxDate(new Date());
								...
								oThes.getOwnerComponent().getModel("oDataModelFondoFijo").read("/ToleranciaSet(Gjahr='" + oModelDetailDocument.Gjahr + "',Monat='" + dFechFactTol + "')", {
								...
								if (oModelDetailDocument.Belnr !== "" && oModelDetailDocument.Belnr !== undefined) {
									var modeloSol = {};
                                   // oThes.loadCentroCostoList();
									oThes.fnCargarDataComboSunat();
									oThes.fnCargarDataOperacion();
								
									modeloSol.oFondo = oModelDetailDocument.sZfondoCompleto;
									modeloSol.oFecDoc = oThes.getView().byId("idFecha").getValue();
									modeloSol.oOperacion = oModelDetailDocument.sZcatCompleto.substr(0, 4);
									modeloSol.oCuenMayor = oModelDetailDocument.Hkont;
									modeloSol.oSol = oModelDetailDocument.Belnr;
									...
									modeloSol.oRef = "Gasto." + oModelDetailDocument.Belnr;
        ...
        validacionHabidoNoHallado: function (sRuc){
            var oThes = this;
            sap.ui.core.BusyIndicator.show(0);
            var  oModel =  oThes.getOwnerComponent().getModel("oDataModelFondoFijo");
            oModel.read("/zruc_nhSet(IpRuc="+"'"+sRuc+"'"+")", {
				//filters: aFilter,
				success: function (result, status, xhr) {
					sap.ui.core.BusyIndicator.hide();
                    if(result.Message.length > 0){
                    return MessageToast.show("Proveedor ingresado con estado de “No Habido/No Hallado");
                    }
				},
		...
		onInit: function () {
			...
			this.oRouter.getTarget("crearGastoFondoFijo").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
			this.onRouteMatched();
		},
		...
		usuarioIasAdmin: function () {
			var userIAS = sap.ui.getCore().getModel("userIAS").getData();
			var bol = false;
			if (userIAS.groups != undefined) {
				if (typeof userIAS.groups === "string" && userIAS.groups === "FI_COLB_ADM_FOND_FIJ_ENTR_REND") {
					...
					var bol = userIAS.groups.find(function (oItem) {
		...
		onRouteMatched: function (oEvent) {
			...
			// var temp = this.usuarioIasAdmin() ? true : false;
			// var temp2 = {
			...
			// 	"isAdmin": temp
			// };
			// this.oGeneral = new JSONModel(temp2);
				...
				oModel.read("/" + sPath, {
					success: function (result) {
						...
						if (result.EpReturn) {
                            ...
                            that.validacionHabidoNoHallado(val);
						} else {
		...
		onCloseDialogAttachDevolucion: function () {
			...
			that._dialogAttachDevolucion.destroy();
		},
		...
		fnCargarDataOperacion: function () {
			...
			oModel.read("/OperacionSet", {
				...
				success: function (result, status, xhr) {
                    ...
                    var sCentroCosto = sap.ui.getCore().getModel("InfoIas").Kostl;
                    oThes._oModelHeaders = {
                        "bukrs": sSociedad
                    };
                    oThes.getOwnerComponent().getModel("oUtilitiesModel").read("/zobjcostoSet", {
                        async:false,
                        headers: oThes._oModelHeaders,
                        success: function (res) {
                            if (res.results.length > 0) {
                                ...
                                if (element.length === 1) {
                                    console.log(element);
                                    oThes.getView().getModel().setProperty("/KostlKtext", element[0].Kostl + " - " + element[0].Ktext);
                                    oThes.oVariablesJSONModel.setProperty("/Kostl/code", element[0].Kostl);
                                    oThes.oVariablesJSONModel.setProperty("/Kostl/name", element[0].Ktext);
                                }
                                oThes.oVariablesJSONModel.setProperty("/objetosCosto/", res.results);
                            } else {
                                oThes.oVariablesJSONModel.setProperty("/objetosCosto/", "");
                            }
                        },
                        error: function (err) {
		...
		onMostrarPorCaja: function () {
			var that = this;
			console.log("Caja Reembolso-> " + $.cajaReembolso)
			var InfoIas = sap.ui.getCore().getModel("InfoIas");
			if (InfoIas.Zfondo !== undefined) {
				$.cajaSolicitante = InfoIas.Zfondo;
			} else {
				$.cajaSolicitante = $.cajaReembolso;
			}
			console.log("Caja del Solicitante-> " + $.cajaSolicitante)
			if ($.cajaSolicitante === $.cajaReembolso) {
				$.isCajaReembolso = true;
				that.getView().byId("idElementReembolso").setVisible(true);
			} else {
				$.isCajaReembolso = false;
				that.getView().byId("idElementReembolso").setVisible(false);
			}
		},

		onLoadInfoFondoFijo: function () {
			var oModel = this.getOwnerComponent().getModel("oDataModelFondoFijo"),
				oModelDetailDocument = this.getOwnerComponent().getModel("oModelDetailDocument");

			var InfoIas = sap.ui.getCore().getModel("InfoIas");
			sap.ui.core.BusyIndicator.show(0);
			oModel.read("/FondoFijoSet(Bukrs='" + InfoIas.Bukrs + "',Zfondo='" + oModelDetailDocument.Zfondo + "')", {
				success: function (result, status, xhr) {
		...
		fnPresionarBoleta: function (oEvent) {
			...
			var adj = sap.ui.getCore().getModel("aTablaDetGasto")[idx].Imagen;
		...
		fnLimpiarCamposCrearGastos: function () {
			...
			//oIdCheckDevol.setSelected(false);///JORDAN COMENTADO SPRINT 3
		...
		fnAgregarDetGasto: function () {
			...
			//	aDetGasto.Wrbtr = this.calcularImporteIGV(aDetGasto.Wrbtr, aDetGasto.Mwskz);
				...
				var aFilaJGasto = {};
				if (aDetGasto.Kstrg == "F") {
					aFilaJGasto.Aufnr = this.getView().byId("idObjCost").getValue().split(" ")[0];
					aFilaJGasto.Kostl = "";
					aFilaJGasto.oKostl = "";
				} else {
					aFilaJGasto.Aufnr = "";
					aFilaJGasto.Kostl = this.getView().byId("idObjCost").getValue().split(" ")[0]; //oIdCeco.getValue();
					aFilaJGasto.oKostl = "";
				}
				...
				if (Number(aFilaJGasto.Wrbtr) <= Number(iImporteTotalSolicitud) || $.isCajaReembolso) {
					...
					if (aFilas.length === 0) {
						...
						this.getView().byId("idImporteTotalIGV").fireChange({ value: aFilaJGasto.Wrbtr });
						//	sap.ui.getCore().setModel(oJGasto, "oJGasto");
					} else {
						var sSuma = 0;
						...
						if (sSuma <= Number(iImporteTotalSolicitud) || $.isCajaReembolso) {
							...
							this.getView().byId("idImporteTotalIGV").fireChange({ value: sSuma });
							//		sap.ui.getCore().setModel(oJGasto, "oJGasto");
						} else {
				...
				if (aFilas.length === 0) {
					...
					this.getView().byId("idImporteTotalIGV").fireChange({ value: impTotal });
				} else {
					var sSuma = 0;
					$.each(aFilas, function (key, value) {
						sSuma += Number(value.Wrbtr);
		...
		fnGrabarGasto: function () {
			...
			// 		MessageBox.alert("Debe seleccionar el checkbox y agregar una devolución");
			// 		//LR 30/12
			// 		this.getView().byId("btnGrabar").setEnabled(true);
			// 		return;
			// 	}
			...
			// 			MessageBox.alert("Debe seleccionar el checkbox de reembolso.");
			// 			//LR 30/12
			// 			this.getView().byId("btnGrabar").setEnabled(true);
			// 			return;
			// 		}
			...
			aJSolicitud.Segment = "";
			aJSolicitud.Parked_document = oModelDetailDocument.Belnr;
			...
			aFilaGrabaGasto.Bldat = Utils.fnFormatearFechaSAP(Utils.fnFormatearFechaVista(oModelDetailDocument.Bldat)) + "T" + dFechaActual.getHours() +
				":" + dFechaActual.getMinutes() + ":" + dFechaActual.getSeconds();
			...
			aFilaGrabaGasto.Augdt = Utils.fnFormatearFechaSAP(Utils.fnFormatearFechaVista(dFechaActual, false)) + "T" + dFechaActual.getHours() +
				":" + dFechaActual.getMinutes() + ":" + dFechaActual.getSeconds();
		...
		fnImportDevolucion: function (oEvent) {
			var route = sap.ui.getCore().getModel("route");
			var RucSociedad = sap.ui.getCore().getModel("InfoIas");
			...
			var sRoute = $.ambiente + "/" + route.carpetaSociedad + "/" + "FONDOFIJO" + "/" + "GASTOS";
			var that = this;

			DS.getCreateFolder(sRoute, sCarpetaGasto).then(oResult => { ///Crea el folder de GAsto
				console.log("Se agrego Carpeta");
				sRoute = sRoute + "/" + sCarpetaGasto;
				DS.getCreateFolder(sRoute, "CABECERAGASTO").then(oResult2 => {
					console.log("Se Agrego CABECERAGASTO");
				});
			});
		},
        ...
        fnSubirObservacionDS: function (oEntrada) {
			...
			return new Promise((resolve, reject) => {
				...
				}).valueHigh}/${oVariablesGlobales.carpetaFondoFijo}/${oVariablesGlobales.carpetaGasto}/${sSolicitud}`;
		...
		fnSubirArchivos: function () {
			...
			if (aFilaDetGasto.Imagen === null || typeof (aFilaDetGasto.Imagen) === "undefined") {
				...
				fileUploadImagenes[aFilaDetGasto.Nrpos] = aFilaDetGasto.Imagen;
				...
				param.Nrpos = aFilaDetGasto.Nrpos;
		...
		fnImport: function (dt) {
			...
			var aFiles = sap.ui.getCore().getModel("fileUploadImagenes");
			var sDireccion;

			//var RucSociedad = sap.ui.getCore().getModel("RucSociedad");
			//Url Estatico
			var RucSociedad = sap.ui.getCore().getModel("InfoIas");
			var route = sap.ui.getCore().getModel("route");

			var sCarpetaGasto = dt.Belnr + dt.Bukrs + dt.Gjahr; //sNumeroDoc+sSociedad+sEjercicio
			//var sRoute = ambiente + "/" + route.carpetaSociedad + "/" + "FONDOFIJO" + "/" + "GASTOS";
			var sRoute = $.ambiente + "/";
			var that = this;
			//sRoute = sRoute + "/" + sCarpetaGasto;
			...
			} else if (dt.Bukrs === "2000") {
				sRoute = sRoute + "20602670130" + "/" + "FONDOFIJO" + "/" + "GASTOS" + "/" + sCarpetaGasto;
			}

			//JORDAN SPRINT 3

			DS.getCreateFolder(sRoute, dt.Nrpos).then(oResult => { ///Crea el folder de posicion y Agrega los elementos
				//Agrego path a cada elemento
				for (var i = 0; i < aFiles[dt.Nrpos].length; i++) {
					aFiles[dt.Nrpos][i].path = sRoute + "/" + dt.Nrpos;
				};
				//Promesa asincrona para agregar todos los archivos en la carpeta de solicitud
				sap.ui.core.BusyIndicator.show(0);
				Promise.all(aFiles[dt.Nrpos].map(function (file) {
					DS.addFile(file.path, file).then(oResult2 => {
						console.log("Se agrego archivo");
						sap.ui.core.BusyIndicator.hide();
					}).catch(function (error) {
						console.log("No se pudo subir archivos: ", error);
						sap.ui.core.BusyIndicator.hide();
					});
				})).then((sResolve) => {
					...
					sap.m.MessageBox.success("Gasto creado correctamente con el numero: " + dt.Belnr);
					//that.fnWorkFlow(aEntrada, "G");
		...
		fnStartInstanceG: function (token, oData) {
			...
			return new Promise((resolve, reject) => {
				...
				//PARA MANDAR LA DESCRIPCION DE LOS OBJETOS DE COSTO Y ORDENES***////
				var oTipoObjCosto = "",
					oObjCosto = "";
				var idTipoObj = that.getView().byId("idTipoObj").getValue().split(" - ")[0];
				var idObjCost = that.getView().byId("idObjCost").getValue();

				var KSTRGModelData = sap.ui.getCore().getModel("KSTRGModelFF").getData();

				for (var count = 0; count < KSTRGModelData.length; count++) {
					if (KSTRGModelData[count].Rstyp == idTipoObj) {
						oTipoObjCosto = KSTRGModelData[count].Rstyp + " - " + KSTRGModelData[count].Rsttx;
					}
				}
				...
				if (idTipoObj == "K") {
					var KOSTLModelData = sap.ui.getCore().getModel("KOSTLModelFF").getData();
					for (var count = 0; count < KOSTLModelData.length; count++) {
						if (KOSTLModelData[count].Kostl == idObjCost) {
							oObjCosto = KOSTLModelData[count].Kostl + " - " + KOSTLModelData[count].Ktext;
						}
					}
				} else {
					var AUFNRModelData = sap.ui.getCore().getModel("AUFNRModelFF").getData();
					for (var count = 0; count < AUFNRModelData.length; count++) {
						if (AUFNRModelData[count].Aufnr == idObjCost) {
							oObjCosto = AUFNRModelData[count].Aufnr + " - " + AUFNRModelData[count].Ktext;
						}
					}
				}
				aData.CuentaDeMayor = that.getView().getModel().getData().oCuenMayor;
				...
				var sImporteRendido = that.getView().byId("idImporteS").getValue().replace(",", "");
				...
				var ZcatCompleto = that.getView().byId("idOperacionCab").getValue();
				var oModelDetailDocument = that.getOwnerComponent().getModel("oModelDetailDocument");
				var Moneda = that.getView().byId("idWaers").getValue();
				var ReferenciaGeneral = that.getView().byId("idReferencia").getValue();
				...
				aData.Kstrg = that.getView().byId("idTipoObj").getValue().split(" - ")[0];
				if (aData.Kstrg == "F") {
		...
		onIrDetalleSolicitud: function () {
			...
			// 	this.doNavigateSolicitud("detailFondoFijo", oModelDetailDocument, fnResolve, "");
		...
		formatFechaTable: function (oEvent) {
			if (oEvent) {
				var anio = oEvent[0] + oEvent[1] + oEvent[2] + oEvent[3];
				var mes = oEvent[4] + oEvent[5];
				var dia = oEvent[6] + oEvent[7];
				return (dia + "-" + mes + "-" + anio)
			}
		},
		formatFechaSunatTable: function (oEvent) {
			if (oEvent) {
				var fecha = oEvent.replace(".", "-").replace(".", "-");
		...
		obtenerTotales: function () {
			var oModelTable = this.getView().byId("idTablaDetGastos").getModel();
			var oDataTable = oModelTable.getData();
			var importeTotalSinIGV = 0;
			var importeTotalConIGV = 0;
			for (var i = 0; i < oDataTable.length; i++) {
				importeTotalSinIGV = importeTotalSinIGV + parseFloat(oDataTable[i].ImporteSinIGV);
				importeTotalSinIGV = this.roundToTwo(importeTotalSinIGV);
				importeTotalConIGV = importeTotalConIGV + parseFloat(oDataTable[i].Wrbtr);
				importeTotalConIGV = this.roundToTwo(importeTotalConIGV);
			}

			var IGV = importeTotalConIGV - importeTotalSinIGV;
			IGV = this.roundToTwo(IGV);

			this.getView().byId("IGVTOTAL").fireChange({ value: IGV });
			this.getView().byId("idImporteTotal").fireChange({ value: importeTotalSinIGV });
			this.getView().byId("idImporteTotalIGV").fireChange({ value: importeTotalConIGV });
		},
