  ...
  function (Log, Filter, FilterOperator) {
    ...
    return {
      ...
      sharepoint: {
         ...
         * @returns {Promise<boolean>} Promise to boolean result
         */
        _createFolderAndSaveFilesRecursive: function (sPath, aFiles, bDelete) {
          return new Promise((resolve, reject) => {
            if (aFiles.length > 0) {
              const oFile = aFiles.pop();
              // Create file's folder
              this.createFolder(sPath, oFile.folderName)
                .then((result) => {
                  if (bDelete) {
                    // Deletes all files from folder
                    return this.deleteFilesFromFolder(sPath, oFile.folderName);
                  } else {
                ...
                .then(() => {
                  // Saves file
                  return this._saveFile(sPath, oFile);
                })
                .then(() => {
                  // Recursive call with the remaining files
                  resolve(this._createFolderAndSaveFilesRecursive(sPath, aFiles, bDelete));
                })
                .catch((error) => {
                  Log.error("[UTILS] Sharepoint - _createFolderAndSaveFiles", error.responseText);
                  reject(error);
                });
            } else {
              // Recursion exit condition
         ...
         * @returns {Promise<boolean>} Promise to boolean result
         */
        saveFiles: function (sPath, aFiles, options) {
          const sDelete = options ? options.delete_existing_files : false;

          return new Promise((resolve, reject) => {
            aFiles = aFiles.slice();

            // Creates root folder
            this.createFolder(sPath, "")
              .then((result) => {
                resolve(this._createFolderAndSaveFilesRecursive(sPath, aFiles, sDelete));
              })
              .catch((error) => {
                Log.error("[UTILS] Sharepoint - saveFiles", error.responseText);
                reject(error);
              });
          });
        },
