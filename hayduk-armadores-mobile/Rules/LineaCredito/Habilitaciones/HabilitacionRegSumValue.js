 ...
 */
export default async function HabilitacionRegSumValue(clientAPI) {
    ...
    try {
        //let spEspecie = clientAPI.evaluateTargetPath("#Page:Habilitaciones/#Control:spEspecie").getValue();
        let spTemporada = clientAPI.evaluateTargetPath("#Page:Habilitaciones/#Control:spTemporada").getValue();
        ...
        sTemporadaFilter = `$filter=IDEspecie eq '01' and IDTipo eq 'HABTEMP' and IDTemporada eq '${spTemporada}'`;


        //$filter=IDRuc eq '20487379353'
    } catch (error) {
        var spTemporada = appSettings.getString("TemporadaHabilitacion");
        if (spTemporada == "" || spTemporada == undefined)
            spTemporada = await getTemporada(clientAPI);
        sTemporadaFilter = `$filter=IDEspecie eq '01' and IDTipo eq 'HABTEMP' and IDTemporada eq '${spTemporada}'`;

    }


    //let containerProxy = clientAPI.getPageProxy().getControl('stHabilitacion');
    //let sotHabilitacion = containerProxy.getControl('SectionObjectTable0');
    //let specifierHabilitacion = sotHabilitacion.getTargetSpecifier();
    //let qo1 = specifierHabilitacion.getQueryOptions();
    ...
    return clientAPI.read(
        ...
        if (oData.length > 0) {
            ...
            return clientAPI.read(
                ...
                //clientAPI.getPageProxy().getClientData().HabilitacionRegSumMonto = iMonto;
