	...
	return {
		...
		readContext: function (oComponent, taskId) {
			...
			var promise = new Promise((resolve, reject) => {
				$.ajax({
					url: this._getAppModulePath(oComponent) + "/bpmworkflowruntime/v1/task-instances/" + taskId + "/context",
					method: "GET",
		...
		triggerComplete: function (oComponent, taskId, stage, oContext, statusIsntace) {
			var promise = new Promise((resolve, reject) => {
				$.ajax({
					url: this._getAppModulePath(oComponent) + "/bpmworkflowruntime/v1/xsrf-token",
					method: "GET",
					headers: {
						"X-CSRF-Token": "Fetch"
					},
					success: (result, xhr, data) => {
						...
						$.ajax({
							url: this._getAppModulePath(oComponent) + "/bpmworkflowruntime/v1/task-instances/" + taskId,
							method: "PATCH",
							contentType: "application/json",
