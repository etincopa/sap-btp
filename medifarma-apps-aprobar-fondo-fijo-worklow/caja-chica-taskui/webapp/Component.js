sap.ui.define([
	...
	"cajachicataskui/model/models",
	"cajachicataskui/model/ContextModel",
	"sap/m/Button",
	"sap/m/Dialog",
	"sap/m/Label",
	"sap/m/MessageToast",
	...
	return UIComponent.extend("cajachicataskui.Component", {
		...
		_callbackAction: function (oDataModel, action) {
			...
			if (action === "confirm") {
            //Inicio Popup - Motivo Aprobacion
				...
				var dialog = new Dialog({
					...
					beginButton: new Button({
						...
						press: function (oEvent) {
							...
							var sText = oEvent.getSource().getParent().getContent()[1].getValue();
							var oData = that.getModel("data").getData();
				...
				dialog.open();
				//Fin Popup Motivo de aprobacion
				...
				var dialog = new Dialog({
					...
					content: [
						new Label({
							text: "¿Cuál es su motivo de rechazo?"
						}),
						new TextArea({
							liveChange: function (oEvent) {
								var sText = oEvent.getParameter("value");
					...
					beginButton: new Button({
						...
						press: function (oEvent) {
							...
							//COMENTADO JORDAN
							if (oData.context.Type == "G") {
								var aResult = that.getModel("modelDetalle").getData().results;

								// 	var aPromise = [];

								that.onUpdateDetalle(aResult).then((oResponse) => {
									...
									// 		that._checkAction(_checkAction, action, oDataModel, "CANCELED");
									oDataModel.oData.context.MotRechz = sText;
									_checkAction = that._checkRejectData(oDataModel.getData());
									that._checkAction(_checkAction, action, oDataModel);
									dialog.close();
		...
		*/
		...
		onUpdateDetalle: function (oItem, cont = 0) {
			...
			return new Promise((resolve, reject) => {
				...
				var oModelCajaChica = this.getModel("oDataModelCajaChica");
				...
				const sPath = oModelCajaChica.createKey("/DetGastoSet", {
				...
				oModelCajaChica.update(`${sPath}`, oItem[cont], {
