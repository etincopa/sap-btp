...
var NavigationPanelHandler = function() {
	...
	this.createText = function(oConfig) {
		if (oConfig && oConfig.design) {
			delete oConfig.design;
		}
		return new sap.m.Title(oConfig);
	};
	
	this.createLabel = function(oConfig) {
		var oLabel;
		if (oConfig && oConfig.design === "Bold") {
			delete oConfig.design;
			delete oConfig.labelFor;
			oLabel = new sap.m.Title(oConfig);
		} else {
			oLabel = new sap.m.Label(oConfig);
		}
		return oLabel;
	};
	...
	this.create = function(oChainedControl, oControlProperties) {
		var oControl, oSplitter, sNavigationPanelMode = oControlProperties.property.navigationpanelmode;
		...
		if (sNavigationPanelMode === "LIST_ONLY") {
			...
			oControl.setCenter(this.createCenterArea({
				overflowX: "hidden", overflowY: "hidden", content: [oSplitter]
			}));
			...
			if (oSplitter) {
				oSplitter.addFirstPaneContent(oBorderLayoutLeft);
			}
			...
			var oSplitterH = this.createSplitter({
				splitterOrientation: "Horizontal",
				splitterPosition: "50%",
				minSizeFirstPane: "30%",
				minSizeSecondPane: "30%",
			...
			oSplitterH.addFirstPaneContent(oBorderLayoutColumns);	
			oSplitterH.addSecondPaneContent(oBorderLayoutRows);	
			
			var oldResizeSplitterElements = oSplitterH.resizeSplitterElements;
			oSplitterH.resizeSplitterElements = function() {
				oldResizeSplitterElements.apply(this);
				oBorderLayoutRows.invalidate();
				oBorderLayoutColumns.invalidate();
			};
			
			var oBorderLayoutRightCenter = this.createCenterArea({overflowX: "hidden", overflowY: "hidden", content: [oSplitterH]});
			...
			if (oSplitter) {
				oSplitter.addSecondPaneContent(oBorderLayoutRight);
			}
		}
