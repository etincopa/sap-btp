...
define("zen.rt.components.filterpanel/resources/js/ValueHelpDialog",  [], function() {
	...
	ValueHelpDialog.prototype.setSupportMultiselect = function(bEnabled) {
		...
		//ZEN this._oButtonOk.setVisible(bEnabled);
		return this;
	};
	...
	*/
				...
				if (this.oRows.aKeys) {
					...
					for (j = 0; j < aItems.length; j++) {
						...
						for (j = 0; j < aItems.length; j++) {
							var key = aItems[j];
							for (i = 0; i < this.oRows.aIndices.length; i++) {
								//oContext = this._oTable.getContextByIndex(this.oRows.aIndices[i]);//ZEN
								oContext = this._oTable.getContextByIndex(i);//ZEN
								if (oContext) {
									oRow = oContext.getObject();
				...
				if (oControlEvent.getParameter("type") === sap.m.Tokenizer.TokenChangeType.Removed) {
					...
					if (eventArgs.useDefault) {
						aTokenKeys.forEach(function(sTokenKey) {
							this._changeTableRowSelectionForKey(encodeURIComponent(sTokenKey), false);
						}.bind(this));
					}
	...
	ValueHelpDialog.prototype._removeRangeByKey = function(sKey, isExclude) {
		...
		aKeys.forEach(function(sKey, index, aKeys) {
			...
			if (range.exclude === isExclude && !range._oGrid) {
	...
	ValueHelpDialog.prototype._getKeyFieldByKey = function(sKey) {
		...
		if (this._aRangeKeyFields) {
			...
			this._aRangeKeyFields.some( function(keyField) {
				if (typeof keyField !== "string" && keyField.key === sKey) {
			...
			if (!bIsIncludeRangesValid) {
				...
				sap.m.MessageBox.show(this._sValidationDialogMessage, {
					...
					onClose: function(sResult) {
						if (sResult === sap.m.MessageBox.Action.OK && fnCallback) {
							fnCallback();
						}
						if (sResult === sap.m.MessageBox.Action.CANCEL && fnCancelCallback) {
							fnCancelCallback();
						}
					}
				});
	...
	ValueHelpDialog.prototype._getDialogOffset = function(windowWidth) {
		...
		if ((iWindowWidth > screenSizes.small && iWindowWidth < screenSizes.large) || iWindowWidth >= screenSizes.large) {
			rem = 2;
		}
	...
	ValueHelpDialog.prototype._getFormatedTokenText = function(sKey) {
		...
		var sDisplayKey = oItem["displaykey"];//ZEN
		if (!sDisplayKey) {//ZEN
			sDisplayKey = sTokenText;
		}//ZEN
		if (sTokenText === undefined) {
			...
			if (sTokenText === "") {
				sTokenText = sDisplayKey; //ZEN
			} else {
                if (sDisplayKey === sTokenText) {//ZEN
                    sTokenText = "";//ZEN
                }//ZEN
				// format the token text 
				sTokenText = sap.ui.comp.util.FormatUtil.getFormattedExpressionFromDisplayBehaviour(this.getTokenDisplayBehaviour() ? this.getTokenDisplayBehaviour() : sap.ui.comp.smartfilterbar.ControlConfiguration.DISPLAYBEHAVIOUR.descriptionAndId, sDisplayKey, sTokenText);
			}
		}
		...
		if (oCurrentKeyField) {
			switch (oCurrentKeyField.type) {
				...
				case "numeric":
					...
					oFormatter = sap.ui.core.format.NumberFormat.getFloatInstance(oFloatFormatOptions);
					break;
				case "date":
					oFormatter = sap.ui.core.format.DateFormat.getDateInstance(jQuery.extend({}, oCurrentKeyField.formatSettings, {strictParsing: true}));
					sValue1 = new Date(sValue1);
					sValue2 = sValue2 !== "" ? new Date(sValue2) : null; // sValue2 can be "" string if the operation != BT.
					break;
				case "time":
					oFormatter = sap.ui.core.format.DateFormat.getTimeInstance(jQuery.extend({}, oCurrentKeyField.formatSettings, {strictParsing: true}));
					sValue1 = new Date(sValue1);
					sValue2 = sValue2 !== "" ? new Date(sValue2) : null;
					break;
				case "datetime":
					oFormatter = sap.ui.core.format.DateFormat.getDateTimeInstance(jQuery.extend({}, oCurrentKeyField.formatSettings, {strictParsing: true}));
					sValue1 = new Date(sValue1);
					sValue2 = sValue2 !== "" ? new Date(sValue2) : null;
					break;
				case "boolean":
