	...
	var TextFieldHandler = function() {
		...
		this.init = function(oControl, oControlProperties) {
			...
			if (oControlProperties.text.dataType) {
				var oDataType, oDataTypeFormatter;
				var loFormatOptions = {
						style: "medium", 
						UTC: oControlProperties.text.UTC
					};
				if (oControlProperties.text.dataType === "DATE") {
					oDataType = this.getDate(lText);
					oDataTypeFormatter = sap.ui.core.format.DateFormat.getDateInstance(loFormatOptions);
				} else if (oControlProperties.text.dataType === "TIME") {
					oDataType = this.getTime(lText);
					oDataTypeFormatter = sap.ui.core.format.DateFormat.getTimeInstance(loFormatOptions);
				} else if (oControlProperties.text.dataType === "DATE_TIME") {
					oDataType = this.getDateTime(lText);
					oDataTypeFormatter = sap.ui.core.format.DateFormat.getDateTimeInstance(loFormatOptions);
				}			
				if (oDataType && oDataTypeFormatter) {
					// format the DataType Object
					lText = oDataTypeFormatter.format(oDataType);
				}	
			}
