...
//Overridng the default "_" context nameToUrl method to add in param to allow browser to know when require sdk components get updated
//Note: Only works for default context
require.s.contexts._.realNameToUrl = require.s.contexts._.nameToUrl;
require.s.contexts._.nameToUrl = function() {
    var url = require.s.contexts._.realNameToUrl.apply(this, arguments);
    if (sapbi_page && sapbi_page.m_required_sdk_versions &&
    	arguments[0] && arguments[0].substring(0, "sdk_include/".length) === "sdk_include/") {
    		
    	var sdkExtension = arguments[0].substring("sdk_include/".length);
    	sdkExtension = sdkExtension.substring(0,sdkExtension.indexOf("/"));
    	if (sdkExtension) {
    		//version param will be added already so use & for hash param. 
    		//TODO : review in SP1 
    		return url + '&hash=' +sapbi_page.m_required_sdk_versions[sdkExtension];
    	}
    }
    return url;
}
