	...
	var FioriHelperHandler = function() {
		...
		this.openSaveTileDialog = function(oControlProperties) {
            ...
            var oSimpleForm = new sap.ui.layout.form.SimpleForm({
                ...
                enabled: (oControlProperties.tileTitle && oControlProperties.tileTitle.trim()) ? true : false
                ...
                id: 'settingsDialog',
                title: 'Save Tile',//oResourcesBundle.getText('tileSettingsButtonTitle'),
                contentWidth: '400px',
                content: oSimpleForm,
			...
			oCrossAppNav.getLinks(aGetLinksArgs).done(function(paLinks) {
				...
				paLinks.forEach(function(aLinksForOneObject) {
					aLinksForOneObject[0].forEach(function(oLink) {
						if (oLink.text) {
							//Only take links which have a text
							aIntents.push(oLink);
						}
					});
				});
		...
		this.sendJumpTargetsToRuntime = function(sOnjumptargetsfetchedcommand) {
			if (sOnjumptargetsfetchedcommand && sOnjumptargetsfetchedcommand.length > 0) {
				...
				sOnjumptargetsfetchedcommand = sOnjumptargetsfetchedcommand.replace("__JUMPTARGETS__", sResultJSON);
				var fAction = new Function(sOnjumptargetsfetchedcommand);
				fAction();
			}
		};
					...
					for (var i = 0; i < iLength; i++) {
						...
						if (sName && sName.length > 0 && !oSelectOptions.hasOwnProperty(sName)) {
							if (aSelectionArray[i].dimension.selection) {
								...
								oSelectOptions[sName] = aSelectionArray[i].dimension.selections;
							}
						}
					}
				}
