sap.ui.define([
	...
	return {
		...
		onSaveRequestBP: function (Owner, oData) {
			return new Promise(function (resolve, reject) {
				Owner.create("/BPRequestSet", oData, {
					async: false,
		...
		onGetRequestBP: function (Owner, entidad, parametro) {
			return new Promise(function (resolve, reject) {
				var sPath = Owner.createKey(entidad, {
					BPRequestID: parametro
				});
				Owner.read("/" + sPath, {
					async: false,
					urlParameters: {
						"$expand": "AdditionalData,Emails,Banks,ContactPersons,Companies/WithholdingTaxCodes,PurchasingOrganizations,History,Attachments,BPRequestPhoneSet"
					},
					success: function (result) {
						resolve(result);
					},
