...
], function (JSONModel, BusyIndicator, oDataService) {
    "use strict";
...
*/
        ...
        downloadFile: function (that, sFolder) {
            ...
            jQuery.ajax({
                ...
                if (response.d.results.length > 0) {
                    ...
                    sap.m.MessageToast("No hay archivos")
                }
                console.log(response);
            }).fail(function (error) {
        ...
        getFiles: function (that, aFolders, sAmbient, sProject, sDocumentNumber, sBPRequestID) {

            function getFile(aFolders) {
                if (aFolders.length > 0) {
                    var oFolder = aFolders.pop();
                    var uri = that.uri +
                        `GetFolderByServerRelativeUrl('${sRoot + "/" + sAmbient + "/" + sProject + "/" + sDocumentNumber + "/" + sBPRequestID + "/" + oFolder.Name}')/Files`;
                    //Busca los archivos
                    jQuery.ajax({
                        url: uri,
                        type: "GET",
                    ...
                    if (aFiles.length > 0) {
                        ...
                        jQuery.ajax({
                            ...
                            jQuery.ajax({
                                ...
                                _this.saveSharepointLog(that, oFile,"X", "Guardado correctamente.");
                                saveFiles(aFiles);
                                console.log(file);

                            }).fail(function (error) {
                               _this.saveSharepointLog(that, oFile,"");
                                BusyIndicator.hide();
                                reject(error);
                            });
                            //	}).catch(err => {
        ...
        saveSharepointLog: function (that, oFile, sFlagFileSaved,sMsg) {
            // TODO: Aparentemente no se terminó de implementar el consumo del servicio
            // OData
            
            // let Line = that.getView().getModel("Line");
            // let oData = {
            //     ID_APP: "Pre-registro Factura con OC",
            //     NRO_OC: Line.PO_NUMBER,
            //     GR_HES: Line.DN_NUMBER,
            //     RUC_PROVEEDOR: Line.ItTaxIdNumber,
            //     COD_SOCIEDAD: Line.ItCompanyId,
            //     FECHAHORA_REGISTRO: "",
            //     FOLDER_NAME: oFile.FolderName,
            //     FLAG_FOLDER: "X",
            //     FLAG_FILE_CONVERSION: "",
            //     FULL_PATH: that.sFullPath,
            //     FILE_NAME: oFile.FileName,
            //     FLAG_FILE_SAVED: sFlagFileSaved,
            //    // MSG_ERROR: sMsg
            //    MSG_ERROR: "ok"
            // };

            //BusyIndicator.show(0);
        /*    oDataService.consult(that, "create", "/DocumentsSaveSet", oData, "").then(response => {
               // BusyIndicator.hide();
