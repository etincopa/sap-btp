sap.ui.define(["sap/ui/model/json/JSONModel", "sap/ui/Device"], function (JSONModel, Device) {
    ...
    return {
        ...
        getJsonDataSunat: function (data) {
            let that = this;
            let temp = data.EXTERNALDOCUMENTID.split("-");

            let codComp = temp[0];
            let numeroSerie = temp[1];
            let numero = temp[2];
            let jObject = {
                numRuc: data.ItTaxIdNumber,
                codComp: codComp,
                numeroSerie: numeroSerie,
                numero: numero,
                fechaEmision: data.INVOICEDATE_TXT,
                numRucAcreedor: data.rucAcreedor,
            };
            let monto_ = data.VALUE;
            if (monto_.includes("-")) monto_ = monto_.replaceAll("-", "");
            if (numeroSerie.includes("E") || numeroSerie.includes("F"))
                // Es una doc electrónico
                jObject.monto = monto_.toString();
            return jObject;
        },
        getJsonData: function (numRuc, codComp, nroInvoice, fechaEmision, monto, numRucAcreedor) {
            let that = this;
            codComp = that.tipoDocumento(codComp);
            return {
                numRuc: numRuc,
                codComp: codComp,
