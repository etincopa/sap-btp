...
/* sap.ui.define([
	"com/everis/suppliers/accountstatus/controller/Principal.controller"
], function (Controller) {
	"use strict";
...
sap.ui.define([
	...
	ListItem,Log) {
	"use strict";
	var sURI = "http://localhost:8080/saperp/sap/opu/odata/EPER/SUPPLIERS_SRV/";
	...
	QUnit.module("MainView Controller", {
		beforeEach: function () {
			...
			/* this._oResourceModel = new ResourceModel({
				bundleUrl: sap.ui.require.toUrl("mif/rmd/registro") + "/i18n/i18n.properties"
			}); */
	...
	QUnit.test("Test FORMATTER VALUE", function (assert) {
		//var done = assert.async();
		var oMainViewController = new MainViewController();
		var value = oMainViewController.formatter.onFormatAmount(12345678);

		assert.equal(value, "12,345,678.00", 'Format Amout correct');
	});

	QUnit.test("Test CompanySet odata", function (assert) {
		var done = assert.async();
		this.oModel.read("/CompanySet", null, null, true, function (oData, oResponse) {
			assert.ok(true, "success handler called");
			assert.equal(oResponse.requestUri, sURI + "CompanySet", "request uri does not have parameters");
			done();
		}, function () {
...
/* 	QUnit.test("Test MD_ESTRUCTURA odata", function (assert) {
	...
	QUnit.test("Test MD_ES_UTENSILIO odata", function (assert) {
		var done = assert.async();
		this.oModel.read("/MD_ES_UTENSILIO", null, null, true, function (oData, oResponse) {
			assert.ok(true, "success handler called");
			assert.equal(oResponse.requestUri, sURI + "MD_ES_UTENSILIO", "request uri does not have parameters");
			done();
		}, function () {
			assert.ok(false, "error handler shouldn't be called");
			done();
	...
	QUnit.test("Test MD_ES_ESPECIFICACION odata", function (assert) {
		var done = assert.async();
		this.oModel.read("/MD_ES_ESPECIFICACION", null, null, true, function (oData, oResponse) {
			assert.ok(true, "success handler called");
			assert.equal(oResponse.requestUri, sURI + "MD_ES_ESPECIFICACION", "request uri does not have parameters");
			done();
		}, function () {
			assert.ok(false, "error handler shouldn't be called");
			done();
	...
	QUnit.test("Test MD_RECETA odata", function (assert) {
		var done = assert.async();
		this.oModel.read("/MD_RECETA", null, null, true, function (oData, oResponse) {
			assert.ok(true, "success handler called");
			assert.equal(oResponse.requestUri, sURI + "MD_RECETA", "request uri does not have parameters");
			done();
		}, function () {
			assert.ok(false, "error handler shouldn't be called");
			done();
	...
	QUnit.test("Test MD_ES_RE_INSUMO odata", function (assert) {
		var done = assert.async();
		this.oModel.read("/MD_ES_RE_INSUMO", null, null, true, function (oData, oResponse) {
			assert.ok(true, "success handler called");
			assert.equal(oResponse.requestUri, sURI + "MD_ES_RE_INSUMO", "request uri does not have parameters");
			done();
		}, function () {
			assert.ok(false, "error handler shouldn't be called");
			done();
	...
	QUnit.test("Test RMD_USUARIO odata", function (assert) {
		var done = assert.async();
		this.oModel.read("/RMD_USUARIO", null, null, true, function (oData, oResponse) {
			assert.ok(true, "success handler called");
			assert.equal(oResponse.requestUri, sURI + "RMD_USUARIO", "request uri does not have parameters");
			done();
		}, function () {
			assert.ok(false, "error handler shouldn't be called");
			done();
